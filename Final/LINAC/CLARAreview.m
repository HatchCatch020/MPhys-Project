...
BPMnoise        = linspace(40,90,6)*1e-6;   % unit
FocusingError   = linspace(10,80,8)*1e-2;   % unit
...
std_rmsBPMvals_C    = zeros(numel(BPMnoise),1);
std_rmsBPMvals_ML   = zeros(numel(BPMnoise), numel(ML_numobs));
...
for i = 1:numel(BPMnoise)
    ...
    std_rmsBPMvals_C(i)   =   std(rmsBPMvals_C(i,1,:));
    for j=1:numel(ML_numobs) 
        mean_rmsBPMvals_ML(i,j) = mean(rmsBPMvals_ML(i,j,:));
        std_rmsBPMvals_ML(i,j)  =  std(rmsBPMvals_ML(i,j,:));
    end
end

figure(1)
hold off
errorbar(BPMnoise,1e3*mean_rmsBPMvals_C(:),1e3*std_rmsBPMvals_C(:), '--+r', 'DisplayName', 'Conventional')
hold on
for j=1:numel(ML_numobs)
    errorbar(BPMnoise,1e3*mean_rmsBPMvals_ML(:,j),1e3*std_rmsBPMvals_ML(:,j), '-.x', 'DisplayName', sprintf('ML (NumObs = %d)', ML_numobs(j)))
end
xlabel('BPM Noise')
